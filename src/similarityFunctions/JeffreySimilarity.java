...
/**
 * Implementa a Diverg?ncia de Jeffrey.
 * @author Vagner M. Gon?alves
 */
public class JeffreySimilarity implements ISimilarity {
	...
	public double computeSimilarity(double[] vModel, double[] vTest) {
		...
		for(int i = 0; i < vectorOfExtractors.size(); i++) {
			...
			if(m == 0.0) {
				...
				distance += ( (d1 * Math.log(d1 / m)) + (d2 * Math.log(d2 / m)) );
			}
		}
		return distance;
	}
